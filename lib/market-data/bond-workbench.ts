...
function archiveReasonFor(term: BondTermSummary, view: BondMarketView, asOfDate: string): BondArchiveReason | null {
  ...
  if (term.outstandingAmount === "0" || view.outstandingAmount === "0") return "balance_exhausted";
  return null;
}
...
function parseView(value: unknown, name: string): BondMarketView {
  ...
  const result = cloneJson(view) as Record<string, unknown>;
  readBondCode(result.bondCode, `${name}.bondCode`);
  readText(result.issuerCode, `${name}.issuerCode`);
  readText(result.bondName, `${name}.bondName`);
  for (const key of ["cbPriceDate", "stockPriceDate", "conversionPriceEffectiveDate", "valuationDate", "outstandingDataDate", "institutionDataDate", "maturityDate", "nextPutDate", "nextEventDate"] as const) readOptionalDate(result[key], `${name}.${key}`);
  for (const key of ["cbClose", "stockClose", "currentConversionPrice", "valuationCbClose", "valuationStockClose", "conversionValue", "outstandingAmount", "outstandingReductionRate", "remainingUnits", "remainingRatio", "dailyTurnoverRate"] as const) readOptionalSignedDecimal(result[key], `${name}.${key}`);
  if (typeof result.cbTradeUnits !== "string" || !/^-?\d+$/.test(result.cbTradeUnits)) throw new TypeError(`${name}.cbTradeUnits is invalid`);
  for (const key of ["daysToMaturity", "daysToNextEvent"] as const) if (!Number.isInteger(result[key])) throw new TypeError(`${name}.${key} is invalid`);
  if (result.daysToNextPut !== null && !Number.isInteger(result.daysToNextPut)) throw new TypeError(`${name}.daysToNextPut is invalid`);
  if (!["redemption", "put", "maturity"].includes(result.nextEventType as string)) throw new TypeError(`${name}.nextEventType is invalid`);
  if (!["complete", "partial", "date_mismatch"].includes(result.dataQuality as string) || typeof result.staleCbPrice !== "boolean") throw new TypeError(`${name} quality is invalid`);
  assertDenseArray(result.missingReasons, `${name}.missingReasons`);
  if (!(result.missingReasons as unknown[]).every((reason) => typeof reason === "string" && reason !== "")) throw new TypeError(`${name}.missingReasons is invalid`);
  return result as unknown as BondMarketView;
}
...
function cloneRecord(value: BondWorkbenchRecord): BondWorkbenchRecord { return parseRecord(value, "previous bond workbench record"); }
function cloneEvent(value: BondWorkbenchEvent): BondWorkbenchEvent { return { ...value }; }
function readBondCode(value: unknown, name: string): string { const text = readText(value, name); if (!/^\d{5,6}$/.test(text)) throw new TypeError(`${name} is invalid`); return text; }
function readText(value: unknown, name: string): string { if (typeof value !== "string" || value.trim() === "") throw new TypeError(`${name} must be a non-empty string`); return value.trim(); }
function readOptionalText(value: unknown, name: string): string | null { return value === null ? null : readText(value, name); }
function readDate(value: unknown, name: string): string { const text = readText(value, name); assertDate(text, name); return text; }
function readOptionalDate(value: unknown, name: string): string | null { return value === null ? null : readDate(value, name); }
function readOptionalDecimal(value: unknown, name: string): string | null { if (value === null) return null; const text = readText(value, name); if (!DECIMAL.test(text)) throw new TypeError(`${name} is invalid`); return text; }
function readOptionalSignedDecimal(value: unknown, name: string): string | null { if (value === null) return null; const text = readText(value, name); if (!SIGNED_DECIMAL.test(text)) throw new TypeError(`${name} is invalid`); return text; }
function readDates(value: unknown, name: string): string[] { assertDenseArray(value, name); return value.map((item, index) => readDate(item, `${name}[${index}]`)); }
function assertTimestamp(value: unknown, name: string): asserts value is string { if (typeof value !== "string" || !isIsoDateTime(value)) throw new TypeError(`${name} must be a valid ISO timestamp`); }
function assertDate(value: unknown, name: string): asserts value is string { if (typeof value !== "string" || !isIsoDate(value)) throw new TypeError(`${name} must be a valid ISO date`); }
function requireRecord(value: unknown, name: string): Record<string, unknown> { if (value === null || typeof value !== "object" || Array.isArray(value)) throw new TypeError(`${name} must be an object`); return value as Record<string, unknown>; }
function assertExactKeys(record: Record<string, unknown>, expected: readonly string[], name: string): void { const keys = Reflect.ownKeys(record); if (keys.length !== expected.length || !keys.every((key) => typeof key === "string" && expected.includes(key) && Object.prototype.propertyIsEnumerable.call(record, key))) throw new TypeError(`${name} keys do not match the verified contract`); }
function assertDenseArray(value: unknown, name: string): asserts value is unknown[] { if (!Array.isArray(value)) throw new TypeError(`${name} must be a dense array`); const keys = Reflect.ownKeys(value); if (keys.length !== value.length + 1 || !keys.includes("length") || !Array.from({ length: value.length }, (_, index) => Object.prototype.propertyIsEnumerable.call(value, String(index)))) throw new TypeError(`${name} must be a dense array`); }
function cloneJson(value: unknown): unknown { if (value === null || typeof value === "string" || typeof value === "boolean" || typeof value === "number") return value; if (Array.isArray(value)) { assertDenseArray(value, "nested array"); return value.map(cloneJson); } const record = requireRecord(value, "nested object"); const result: Record<string, unknown> = {}; for (const key of Reflect.ownKeys(record)) { if (typeof key !== "string" || !Object.prototype.propertyIsEnumerable.call(record, key)) throw new TypeError("nested object keys do not match the verified contract"); result[key] = cloneJson(record[key]); } return result; }
