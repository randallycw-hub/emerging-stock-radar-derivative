...
export function resolveTradeState(input: TradeStateInput): TradeStateResult {
  ...
  return freeze({
    state: latestTradeDate === dataDate ? "TRADED_TODAY" : "NO_TRADE_TODAY",
...
export function calculateRollingMetrics(points: readonly RollingPoint[]): RollingMetrics {
  ...
  const volumeRatio = current?.volume !== null && average5 !== null && average5 > 0
    ? round(current.volume / average5)
    : null;
  const amountChange = current?.amount !== null && averageAmount20 !== null && averageAmount20 > 0
...
function average(values: readonly (number | null)[], requiredSize: number): number | null {
  ...
  return round(values.reduce((sum, value) => sum + (value ?? 0), 0) / requiredSize);
}
