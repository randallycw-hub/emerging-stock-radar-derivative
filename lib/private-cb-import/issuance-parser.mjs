...
export function parseIssuanceRows({ fileName, rows } = {}) {
  ...
  const records = rows.slice(header + 1).filter((row) => text(row?.[2]) !== "").map((row) => {
    const record = {
      ...
      announcementDate: optionalDate(row[7], "announcement date"),
      filingDate: optionalDate(row[8], "filing date"),
      effectiveDate: optionalDate(row[9], "effective date"),
      marketingEvent: optional(row[10]),
      premiumRate: number(row[11], "premium rate"),
      conversionPrice: number(row[12], "conversion price"),
      listingDate: optionalDate(row[13], "listing date"),
      asoSplitDate: optionalDate(row[14], "ASO split date"),
...
function optionalDate(value, label) {
  ...
  if (!isIsoDate(date)) throw new TypeError(`${label} is invalid`);
  return date;
}
