...
export function parseCbasRows({ quoteRows, dueRows, stopRows } = {}) {
  ...
  const quoteRecords = uniqueRows(quoteRows.slice(quoteHeader + 1), "CBAS quote", (row) => mapQuote(row));
...
function uniqueRows(rows, label, mapper, identity = (record) => record.bondCode) {
  const seen = new Set();
  return rows.filter((row) => Array.isArray(row) && row.some((cell) => value(cell) !== "")).map((row) => {
    const record = mapper(row);
    const key = identity(record);
    if (seen.has(key)) throw new TypeError(`${label} contains duplicate canonical key: ${key}`);
    seen.add(key);
