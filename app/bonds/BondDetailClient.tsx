...
export default function BondDetailClient({ bondCode }: { bondCode: string }) {
  ...
  if (state === "unavailable") return <main className="bond-detail-page"><Link href="/bonds">← 返回可轉債契約</Link><section className="bond-detail-state"><h1>正式資料尚未發布</h1><p>目前沒有合格的可轉債發布快照，無法顯示 {bondCode} 的契約數值。</p></section></main>;
  ...
  const fields: Array<[string, string]> = [
    ...
    ["擔保狀態", bond.guaranteeStatus || "—"], ["資料日期", bond.officialDataDate || "—"],
  ];
  return <main className="bond-detail-page"><Link href="/bonds">← 返回可轉債契約</Link><header><span>CONTRACT DETAIL</span><h1>{bond.bondCode || bondCode}</h1><p>{bond.bondName || "未提供債券簡稱"}</p></header><section className="bond-detail-panel"><h2>發行條件與契約欄位</h2><dl>{fields.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></section><p className="bond-detail-disclaimer">本頁僅呈現發布快照的契約資料，不提供成交價格、買賣價、折溢價、理論價格或投資建議。</p></main>;
}
