...
2. `ingestDataset` persists the run, immutable snapshot metadata, and typed dataset records. A response hash determines the snapshot identity; a duplicate hash is rejected as an existing immutable snapshot, while a different response creates a new snapshot and never overwrites the old one.
