...
| 時間 | 工作 |
|---|---|
| 16:15 | FAST：取得可用的盤後候選；只做驗證，不切換正式市場快照 |
| 17:45 | OFFICIAL：官方收盤候選；只有資料日期等於指定台北日期且較前版前進才可切換 |
| 22:30 | 以 `Asia/Taipei` 資料日執行完整市場候選：重新取得完整且已驗證的 11406 名冊、核心條款與核心 CB 盤後行情，再做候選驗證與健康檢查 |
| 次日 07:30 | RECONCILE：只記錄並驗證官方更正；不將更正資料混入一般盤後流程 |
| 週六 10:00 | WEEKLY：確認興櫃上個完整週的最後有效日均價基準 |

若官方來源不保證固定時間，以來源資料日期判斷是否重試，不把某一時刻硬編碼為必定完成。GitHub 排程只執行驗證與候選建立；它不持有部署憑證，也不會發布、切換網站版本或回寫 Git。正式發布仍須經人工核准的 hosting 流程。
