...
export function renderDataCenterBootstrap(status = {}) {
  ...
  const checks = Array.isArray(status?.qa?.checks) ? status.qa.checks : [];
  const timeline = Array.isArray(status?.timeline) ? status.timeline : [];
  const sources = Array.isArray(status?.sources) ? status.sources : [];
  const commonStates = Array.isArray(status?.commonStates) ? status.commonStates : [];
  const unresolved = Array.isArray(status?.incidents?.unresolved) ? status.incidents.unresolved : [];
  const qaCount = checks.filter((check) => check?.passed === true).length;
  const datasetRows = datasets.map((dataset) => `<tr><th scope="row">${sourceLink(dataset.sourceUrl, dataset.label)}<small>${escapeHtml(dataset.source ?? "—")}</small></th><td>${publicDate(dataset.dataDate)}</td><td>${publicDate(dataset.lastSuccessAt)}</td><td>${dataset.recordCount === null ? "—" : escapeHtml(dataset.recordCount)}</td><td>${statusBadge(dataset.status, dataset.statusLabel)}</td><td>${escapeHtml(dataset.statusReason ?? "—")}</td></tr>`).join("") || `<tr><td colspan="6">目前沒有可顯示的資料集狀態。</td></tr>`;
  const datasetCards = datasets.map((dataset) => `<article class="data-health-card"><header><h3>${sourceLink(dataset.sourceUrl, dataset.label)}</h3>${statusBadge(dataset.status, dataset.statusLabel)}</header><dl><div><dt>來源</dt><dd>${escapeHtml(dataset.source ?? "—")}</dd></div><div><dt>資料日</dt><dd>${publicDate(dataset.dataDate)}</dd></div><div><dt>最後成功</dt><dd>${publicDate(dataset.lastSuccessAt)}</dd></div><div><dt>資料量</dt><dd>${dataset.recordCount === null ? "—" : escapeHtml(dataset.recordCount)}</dd></div></dl><p>${escapeHtml(dataset.statusReason ?? "—")}</p></article>`).join("") || `<p class="empty-state">目前沒有可顯示的資料集狀態。</p>`;
  const qaItems = checks.map((check) => `<li><span aria-hidden="true">${check?.passed === true ? "✓" : "!"}</span>${escapeHtml(check?.label ?? "—")}<strong>${check?.passed === true ? "通過" : "未通過"}</strong></li>`).join("") || `<li>目前沒有可顯示的公開資料檢核結果。</li>`;
  const timelineItems = timeline.map((event) => `<li><time>${publicDate(event?.at)}</time><div><strong>${escapeHtml(event?.label ?? "—")}</strong><p>${escapeHtml(event?.detail ?? "—")}</p></div></li>`).join("") || `<li>目前尚無可顯示的更新紀錄。</li>`;
  const sourceItems = sources.map((source) => `<article class="data-source-card"><h3>${escapeHtml(source?.name ?? "—")}</h3><p>${escapeHtml(source?.purpose ?? "—")}</p>${(Array.isArray(source?.urls) ? source.urls : []).map((url) => sourceLink(url, "開啟官方來源")).join("") || "<span>—</span>"}</article>`).join("") || `<p class="empty-state">目前沒有可顯示的官方來源。</p>`;
  const stateItems = commonStates.map((state) => `<article><h3>${escapeHtml(state?.label ?? "—")}</h3><p>${escapeHtml(state?.description ?? "—")}</p></article>`).join("");
  return `<section class="data-center-safe-summary" aria-labelledby="data-center-safe-title"><div class="data-center-system-hero"><div><p class="section-number">SYSTEM STATUS / SAFE SNAPSHOT</p><h2 id="data-center-safe-title">系統資料狀態</h2><p class="data-center-system-message">${statusBadge(statusId, DATA_CENTER_STATUS_LABELS[statusId])} ${escapeHtml(system.statusReason ?? "—")}</p></div><dl class="data-center-safe-facts"><div><dt>市場資料日</dt><dd>${publicDate(status?.dataDate)}</dd></div><div><dt>最後完整更新</dt><dd>${publicDate(system.lastCompleteUpdateAt)}</dd></div><div><dt>最後 QA</dt><dd>${publicDate(system.lastQaAt)}</dd></div><div><dt>正常資料集</dt><dd>${Number.isInteger(system.normalDatasetCount) && Number.isInteger(system.totalDatasetCount) ? `${system.normalDatasetCount} / ${system.totalDatasetCount}` : "—"}</dd></div><div><dt>快照</dt><dd>${escapeHtml(snapshot.current ?? "—")}</dd></div></dl></div><section class="data-center-panel" aria-labelledby="data-health-heading"><header class="section-heading"><div><p class="section-number">01 / DATASET HEALTH</p><h2 id="data-health-heading">Dataset Health</h2><p>以目前已發布快照呈現各公開資料集的資料日、最後成功時間與狀態。</p></div></header><div class="data-health-table-shell"><table class="data-health-table"><thead><tr><th scope="col">資料集</th><th scope="col">資料日</th><th scope="col">最後成功</th><th scope="col">資料量</th><th scope="col">狀態</th><th scope="col">說明</th></tr></thead><tbody>${datasetRows}</tbody></table></div><div class="data-health-cards">${datasetCards}</div></section><section class="data-center-grid"><section class="data-center-panel" aria-labelledby="data-quality-heading"><header class="section-heading"><div><p class="section-number">02 / DATA QUALITY</p><h2 id="data-quality-heading">公開資料檢核</h2></div><strong class="data-quality-count">${qaCount} / ${checks.length}</strong></header><ul class="data-quality-list">${qaItems}</ul><p class="data-quality-time">最後檢核：${publicDate(status?.qa?.completedAt)}</p></section><section class="data-center-panel" aria-labelledby="data-incident-heading"><header class="section-heading"><div><p class="section-number">04 / INCIDENTS & CORRECTIONS</p><h2 id="data-incident-heading">異常與勘誤</h2></div></header>${unresolved.length ? `<ul class="data-incident-list">${unresolved.map((incident) => `<li>${escapeHtml(incident?.summary ?? "—")}</li>`).join("")}</ul>` : '<p class="empty-state">目前沒有未處理的資料異常。</p>'}</section></section><section class="data-center-panel" aria-labelledby="data-timeline-heading"><header class="section-heading"><div><p class="section-number">03 / UPDATE TIMELINE</p><h2 id="data-timeline-heading">更新紀錄</h2></div></header><ol class="data-update-timeline">${timelineItems}</ol></section><section class="data-center-panel" aria-labelledby="data-source-heading"><header class="section-heading"><div><p class="section-number">05 / OFFICIAL SOURCE REGISTRY</p><h2 id="data-source-heading">官方來源</h2></div></header><div class="data-source-registry">${sourceItems}</div></section><section class="data-center-grid"><section class="data-center-panel" aria-labelledby="data-state-heading"><header class="section-heading"><div><p class="section-number">06 / COMMON DATA STATES</p><h2 id="data-state-heading">資料狀態說明</h2></div></header><div class="data-common-states">${stateItems}</div></section><section class="data-center-panel" aria-labelledby="data-methodology-heading"><header class="section-heading"><div><p class="section-number">07 / METHODOLOGY</p><h2 id="data-methodology-heading">方法與定義</h2></div></header><p>可轉債估值、無成交、資料日期與 IPO 階段皆依既有公開資料方法呈現。</p><p><a class="data-methodology-link" href="./methodology.html">查看完整資料方法與計算原則 →</a></p></section></section></section>`;
}

function statusBadge(status, label) {
  const id = Object.hasOwn(DATA_CENTER_STATUS_LABELS, status) ? status : DATA_CENTER_STATUS.ERROR;
  return `<span class="data-status data-status--${escapeHtml(id.toLowerCase())}">${escapeHtml(label ?? DATA_CENTER_STATUS_LABELS[id])}</span>`;
}

function sourceLink(url, label) {
  try {
    const parsed = new URL(url);
    if (parsed.protocol === "https:") return `<a href="${escapeHtml(parsed.href)}" target="_blank" rel="noreferrer">${escapeHtml(label)}</a>`;
  } catch {
    // Deliberately omit unverified links from the public page.
  }
