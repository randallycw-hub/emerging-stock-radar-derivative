...
test("靜態可轉債頁維持精簡公開市場欄位與封存切換", async () => {
  const html = await readFile(new URL("../static-showcase/bonds.html", import.meta.url), "utf8");
  for (const label of ["CB 代碼／名稱", "CB 收盤", "轉換價值", "轉換溢價率", "標的股收盤", "目前轉換價", "流通餘額比例", "下一事件"]) {
    assert.match(html, new RegExp(label));
  }
  assert.doesNotMatch(html.match(/<thead>[\s\S]*?<\/thead>/)?.[0] ?? "", /資料日期/);
  assert.match(html, /id="bond-update-status"/);
